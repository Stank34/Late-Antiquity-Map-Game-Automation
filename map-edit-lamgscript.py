...
class MapGameEditor:
            ...
            for neighbor in data['neighbors']:
                if neighbor in self.topology:
                    ...
                    if connection not in drawn_lines:
                        x2, y2 = int(self.topology[neighbor]['x']) * 3 / 2, int(self.topology[neighbor]['y']) * 3 / 2
    ...
    def refresh_node_color(self, prov_id):
        ...
        if owner in self.player_colors:
            ...
            new_color = "white" # Default color
        fortified = state.get("is_fortified", False)
        city = state.get("has_city", False)
        ...
        self.canvas.itemconfig(f"prov_{p_id}", fill=new_color, width=new_thickness, outline=new_outline)
